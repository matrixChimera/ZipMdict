...
class MDictWriter(object):
	...
	def _build_offset_table(self,d):
		...
		if isinstance(d, dict):
			items = list(d.items())
		elif isinstance(d, collections.defaultdict):
			items = list(d.items())
		else:
			items = [ n for n in d ]

		#pairs=[]
		#for key, record in sorted(d.items(),key=lambda v: (v[0].upper(),v[0].swapcase())): #lower case first
		#	for i in range(len(record)):
		#		pairs.append((key,record[i]))

                #Azure修改过的核心排序代码，确保排序为dot,doT,dOt,Dot,DoT,DOt,DOT
		def sort_key (item):
			text = []
			part = []
			word = item[0].lower()
			word2 = item[0].swapcase()
			for n in word:
				if n.isalnum() or ord(n) >= 128:
					...
					part.append('~')
			return ((''.join(text)), ''.join(part), word, word2)
		...
		for key, record in items:
			for i in range(len(record)):
				...
				#for i in range(1):
				if self._is_mdd:
					record_null = record[i]
					#record_null = record
				else:
					record_null = (record[i]+"\0").encode(self._python_encoding)
...
class _MdxKeyBlock(_MdxBlock):
	...
	def __init__(self, offset_table, compression_type, version):
		...
		#print self._num_entries
		#print self._first_key
		#print self._last_key
		#entries_count+=self._num_entries
		#print self._num_entries
