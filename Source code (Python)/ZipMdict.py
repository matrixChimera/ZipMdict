...
def write_mdict():
    ...
    if write_path.split(os.path.sep)[-1].endswith('mdx'):
        ...
        if '.DS_Store' in files_in_write_path:
            files_in_write_path.remove('.DS_Store')
        # The folder must have at least 1 file:
        if len(files_in_write_path) == 0:
            tkinter.messagebox.showerror('Error', "The number of files in the folder must be >=1.")
        else:
            ...
            # region Call the package, mdict-utils, to zip files into a .mdd file
            current_path = os.getcwd()
            os.system('cd {}'.format(folder_name))
            os.system('mdict -a {} {}'.format(write_path, outfile_name))
            os.chdir(current_path)
            # endregion
            # region Messagebox for reporting outcome:
            tkinter.messagebox.showinfo('Output', 'Write/Zip: \n\t{}\n\nOutput: \n\t{}'.format(write_path, outfile_name))
            # endregion
    else:
        # Modified by 掌上百科@maomaowei5655 in 2020-04-23:
